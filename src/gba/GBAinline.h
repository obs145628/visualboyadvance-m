  ...
  unreadable:
    ...
    if (cpuDmaHack) {
      ...
      if (armState) {
        ...
        return CPUReadHalfWordQuick(reg[15].I) | CPUReadHalfWordQuick(reg[15].I)
                                                     << 16;
      }
    }
    break;
  ...
  if (oldAddress & 3) {
    if (systemVerbose & VERBOSE_UNALIGNED_MEMORY) {
      log("Unaligned word read from: %08x at %08x (%08x)\n", oldAddress,
          armMode ? armNextPC - 4 : armNextPC - 2, value);
    }
  }
#endif
  return value;
...
static inline void CPUWriteMemory(uint32_t address, uint32_t value) {
  ...
  // Syscall edit
  if (address == SYSCALL_TRIGGER_ADDR)
    run_syscall();
...
static inline void CPUWriteByte(uint32_t address, uint8_t b) {
  ...
  case 15:
    if ((saveType != 5) &&
        ((!eepromInUse) | cpuSramEnabled | cpuFlashEnabled)) {
